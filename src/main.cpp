...
void print_last_days(const Eigen::MatrixXd& M,
    ...
    int start = std::max(0, R - ndays);
    std::cout << name << " (last " << ndays << " days, " << R << "x" << C << ")\n";
    for (int r = start; r < R; ++r) {
        std::cout << dates[r] << " Row[" << r << "] head: ";
        for (int j = 0; j < std::min(head, C); ++j) {
            std::cout << M(r, j) << " ";
        }
        std::cout << "\n";
    }
...
int main() {
    ...
    Eigen::setNbThreads(1);  // ✅ 關掉 Eigen 內部多執行緒
    ...
    //data loader
    DataLoader loader("/mnt/c/backup_portfolio_dsa/portfolio_dsa/cpp_core/src");           // 原本是 "."
	...
	// Features summary (last 10 days for each feature)
	print_last_days(loader.feat_Gap(),           loader.dates(), "feat_Gap", 10, 5);
	print_last_days(loader.feat_Mom5(),          loader.dates(), "feat_Mom5", 10, 5);
	print_last_days(loader.feat_Mom10(),         loader.dates(), "feat_Mom10", 10, 5);
	print_last_days(loader.feat_Mom20(),         loader.dates(), "feat_Mom20", 10, 5);
	print_last_days(loader.feat_BIAS(),          loader.dates(), "feat_BIAS", 10, 5);
	print_last_days(loader.feat_BrokerStrength(),loader.dates(), "feat_BrokerStrength", 10, 5);
	print_last_days(loader.feat_Liquidity(),     loader.dates(), "feat_Liquidity", 10, 5);
	print_last_days(loader.feat_TurnoverShare(), loader.dates(), "feat_TurnoverShare", 10, 5);
	print_last_days(loader.feat_Imbalance(),     loader.dates(), "feat_Imbalance", 10, 5);
	print_last_days(loader.feat_GKVol(),         loader.dates(), "feat_GKVol", 10, 5);
	print_last_days(loader.C(),                  loader.dates(), "ClosePrice (C)", 10, 5);
	...
	for (int i=0; i<10; ++i)
 	 std::cout << loader.symbols()[i] << ": " << prediction.rhat_enh(i) << "\n";
