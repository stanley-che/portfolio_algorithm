...
public:
    explicit ProcessExporter(const dlx::DataLoader& dl) : eng_(dl) {}
                                                   ...
                                                   double P0) const
{
    ...
    auto price_row = [&](int day)->Eigen::VectorXd{
        if(day<0 || day>=eng_.dl().C().rows()) return Eigen::VectorXd::Zero(N);
        Eigen::VectorXd v = eng_.dl().C().row(day).transpose();
