...
// ✅ 放在 data_loader.cpp（DataLoader method 區域）
// 回傳：true 表示這個 symbol 在 universe 內（或可加入），false 表示跳過
bool DataLoader::accept_or_register_symbol_(const std::string& sym) {
    auto it = sym2col_.find(sym);
    if (it != sym2col_.end()) return true; // 已在 universe

    // 還沒出現過：看是否允許新增
    if (cfg_.max_symbols > 0 && (int)symbols_.size() >= cfg_.max_symbols) {
        return false; // 超過上限 -> 不加入 universe
    }

    int col = (int)symbols_.size();
    symbols_.push_back(sym);
    sym2col_[sym] = col;

    // stock_info_ 也要同步長度（避免後面 meta/broker 填值越界）
    stock_info_.push_back(StockInfo{sym});
    return true;
}
